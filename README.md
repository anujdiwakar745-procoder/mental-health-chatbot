...
> ⚠️ Note: The backend runs on free-tier hosting, so the first message after inactivity may take 30–50 seconds while the server wakes up. Subsequent messages are fast.
...
cd backend
npm install
# Create a .env file with PORT, MONGO_URI, ML_SERVICE_URL
node server.js
